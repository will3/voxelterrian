...
bool Mask::stop_merge(MaskValue & c, MaskValue & next) {
	return next.v != c.v || next.has_ao() || next.lighting != c.lighting;
}

void Mask::copy_quad(VoxelMesh * buffer, int x, int y, int w, int h, int ao0, int ao1, int ao2, int ao3, int lighting) {
...
void Mask::get_quads(VoxelMesh * mesh) {
	int n = 0;
	MaskValue c;
	int w, h;
	...
	for (int j = 0; j < size; j++) {
		for (int i = 0; i < size; ) {
			...
			// Check AO
			if (c.has_ao()) {
				copy_quad(mesh, j, i, 1, 1, c.ao0, c.ao1, c.ao2, c.ao3, c.lighting);
				i++;
				n++;
				continue;
			}
			...
			// Add Quad
			copy_quad(mesh, j, i, h, w, 0, 0, 0, 0, lighting);
